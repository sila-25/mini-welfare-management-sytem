...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
        ...
        redirect('/modules/contributions/record.php');
    }
    ...
    try {
        ...
        // Validate member
        $stmt = db()->prepare("SELECT id, first_name, last_name, member_number, email FROM members WHERE id = ? AND group_id = ? AND status = 'active'");
        ...
        $sendEmail = isset($_POST['send_email']) && $_POST['send_email'] == '1' && !empty($member['email']);
        
        if ($sendEmail) {
            $subject = "Contribution Receipt - " . $receiptNumber;
            $body = "
                <html>
                <head>
                    <style>
                        ...
                        .header { text-align: center; border-bottom: 2px solid #667eea; padding-bottom: 10px; }
                ...
                <body>
                    <div class='receipt'>
                        ...
                        <p><strong>Dear {$member['first_name']} {$member['last_name']},</strong></p>
                        <p>Thank you for your contribution. Please find the details below:</p>
                        <table width='100%'>
                            <tr><td><strong>Amount:</strong></td><td class='amount'>" . formatCurrency($amount) . "</td></tr>
                            <tr><td><strong>Type:</strong></td><td>" . ucfirst($contributionType) . "</td></tr>
                            <tr><td><strong>Date:</strong></td><td>" . formatDate($paymentDate) . "</td></tr>
                            <tr><td><strong>Method:</strong></td><td>" . ucfirst(str_replace('_', ' ', $paymentMethod)) . "</td></tr>
                        </table>
                        <p>You can view all your contributions by logging into your account.</p>
                        <hr>
                        <small>This is an automated receipt. Please keep it for your records.</small>
                    </div>
                </body>
                </html>
            ";
        ...
        $message = "Contribution recorded successfully! Receipt Number: {$receiptNumber}";
        if ($sendEmail) {
            $message .= " Receipt sent to {$member['email']}";
        }
...
// Get member details if pre-selected
...
if (isset($_GET['member_id'])) {
    ...
    $stmt->execute([$_GET['member_id'], $groupId]);
    $preSelectedMember = $stmt->fetch();
}
                ...
                <div class="card-body">
                    <form action="" method="POST" id="contributionForm">
                        ...
                        <div class="row">
                                ...
                                <select class="form-select select2" name="member_id" required>
                                    ...
                                    <?php foreach ($members as $member): ?>
                                        <option value="<?php echo $member['id']; ?>" 
                                            <?php echo ($preSelectedMember && $preSelectedMember['id'] == $member['id']) ? 'selected' : ''; ?>
                                            data-member-number="<?php echo $member['member_number']; ?>">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control" name="amount" step="0.01" min="0.01" required placeholder="0.00" id="amount">
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 mb-3" id="referenceField" style="display: none;">
                                ...
                                <small class="text-muted">Required for bank transfers and mobile money</small>
                            </div>
                                ...
                                <select class="form-select" name="account_id">
                                    ...
                                    <?php foreach ($accounts as $account): ?>
                                        <option value="<?php echo $account['id']; ?>">
                                            <?php echo htmlspecialchars($account['account_name']); ?> (<?php echo ucfirst($account['account_type']); ?>) - Balance: <?php echo formatCurrency($account['current_balance']); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" name="send_email" id="sendEmail" value="1">
                                    <label class="form-check-label" for="sendEmail">
                                        Send receipt via email to member
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="mt-4">
                            ...
                            <button type="button" class="btn btn-info ms-2" onclick="addAnother()">
            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-center">
                            <h3 class="text-info"><?php echo number_format($todayStats['unique_members'] ?? 0); ?></h3>
    ...
    $('#paymentMethod').change(function() {
        ...
        if (method === 'bank_transfer' || method === 'mobile_money' || method === 'cheque') {
            ...
            if (method === 'bank_transfer') {
                $('input[name="reference_number"]').attr('placeholder', 'Transaction ID / Reference Number');
            } else if (method === 'mobile_money') {
                $('input[name="reference_number"]').attr('placeholder', 'M-PESA Transaction Code');
            } else {
                $('input[name="reference_number"]').attr('placeholder', 'Cheque Number');
            }
        } else {
            $('#referenceField').hide();
...
function addAnother() {
    // Change form action to stay on same page
    $('#contributionForm').attr('action', '');
...
$('#contributionForm').on('submit', function(e) {
    ...
    const referenceNumber = $('input[name="reference_number"]').val();
    ...
    if ((paymentMethod === 'bank_transfer' || paymentMethod === 'mobile_money') && !referenceNumber) {
        e.preventDefault();
        Swal.fire('Error', 'Please enter a reference number for ' + paymentMethod.replace('_', ' '), 'error');
        return false;
    }
